...
public class QuestionController {
    ...
    public List<QuestionResultDTO> findByTechnology(@PathVariable String technology){
        ...
        var toMap = result.stream().map(question -> mapQuesttionToDTO(question)).collect(Collectors.toList());
        
        return toMap;
    }

    static QuestionResultDTO mapQuesttionToDTO(QuestionEntity question){
    ...
    static AlternativesResultDTO mapAlternativeDTO(AlternativesEntity alternativeResultDTO){
        return AlternativesResultDTO.builder().id(alternativeResultDTO.getId()).description(alternativeResultDTO.getDescription()).build();
    }
